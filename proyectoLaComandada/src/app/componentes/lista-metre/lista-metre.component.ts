...
export class ListaMetreComponent implements OnInit {
  ...
  solicitudesObs: Observable<SolicitudMesa[]>;
  ...
  ionViewWillEnter() {
    ...
    this.solicitudesObs = this.listaService.traerSolicitudes().snapshotChanges().pipe( map ( actions => {
      return actions.map( a => {
        const data1 = a.payload.doc.data() as SolicitudMesa;
    ...
    this.solicitudesObs.subscribe( data => {
      this.solicitudes = [];
   ...
   */
 async asignarMesa(soli: SolicitudMesa, estado) {


    if (estado === 'rechazado') {
      this.presentAlertConfirm(soli);
      ...
      if (confirm) {
        ...
        soli.estado = 'aceptado'; // cambio el estado de la solicitud para que quede guardado que se acepto la solicitud de la mesa

        this.listaService.actualizarSolicitud(soli);
 ...
 */
   ...
   */
  async presentAlertConfirm(soli: SolicitudMesa) {
    const alert = await this.alertController.create({
