...
export class HacerPedidoClienteComponent implements OnInit {
  ...
  constructor(private toastController: ToastController,
    public pedidoServi : PedidosClienteService,
  ...
  async iniciar()
  {    
    this.pedidoServi.datosUsuario().subscribe(
      datos =>{
        if(datos[0].pedido == 'si' || datos[0].pedido == 'recibido' || datos[0].pedido == 'verificado')
        {
          this.menu = false;
          this.bebidas = false;
          this.comidas = false;
          this.postres = false;
          this.tengoPedido = true;
        }
      }
    )
    //comida
  ...
  async altaPedido() {
    const alert = await this.alerta.create({
      header: 'Pedido',
      message: '<br>¿Esta seguro que quiere realizar el pedido?',
      buttons: [
        {
          text: 'SI, estoy seguro',
          handler: () => {
            console.log("ACEPTE EL PEDIO");
            this.pedidoServi.agregarPedido(this.pedido);
            this.mensaje("Se envio tu pedido al mozo con exito!");
            //
          }
        ...
        {
          text: 'NO, cancelar pedido',
          role: 'cancel',
          cssClass: 'secondary',
          handler: (blah) => {
            this.mensaje('Se cancelo el pedido');
            }
        }
